...
from ..foi.base import VaccineFOI
...
class BruteForceSEIR(BaseSEIR):
    ...
    foi = xs.foreign(VaccineFOI, 'foi', intent='in')
    ...
    # violates single definition rule; implemented in brute_force.py
    ...
    def run_step(self, step_delta):
        ...
        for v in self.counts.coords['vertex'].values:
            ...
            for a, r in product(self.counts.coords['age_group'].values,
                ...
                # vaccinated to exposed (leaky vaccine model)
                rate_V2EV = self.foi.loc[{
                    'vertex': v,
                    'age_group': a,
                    'risk_group': r,
                    'compartment': 'V'
                ...
                rate_EV2Pa = self.discrete_time_approx(self.sigma) * cts('EV')
                rate_EV2Py = self.discrete_time_approx(self.sigma) * cts('EV')
