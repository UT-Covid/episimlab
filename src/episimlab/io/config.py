...
class ReadV1Config:
    ...
    seed_entropy = xs.foreign(KEYS_MAPPING['seed_entropy'], 'seed_entropy', intent='out')
    sto_toggle = xs.foreign(KEYS_MAPPING['sto_toggle'],
                            'sto_toggle', intent='out')
    t_onset_to_h = xs.foreign(KEYS_MAPPING['t_onset_to_h'], 't_onset_to_h', intent='out')
    tri_h2r = xs.foreign(KEYS_MAPPING['tri_h2r'], 'tri_h2r', intent='out')
    tri_h2d = xs.foreign(KEYS_MAPPING['tri_h2d'], 'tri_h2d', intent='out')
    tri_y2r_para = xs.foreign(KEYS_MAPPING['tri_y2r_para'], 'tri_y2r_para', intent='out')
    tri_py2iy = xs.foreign(KEYS_MAPPING['tri_py2iy'], 'tri_py2iy', intent='out')
    tri_exposed_para = xs.foreign(KEYS_MAPPING['tri_exposed_para'], 'tri_exposed_para', intent='out')
    tri_pa2ia = xs.foreign(KEYS_MAPPING['tri_pa2ia'], 'tri_pa2ia', intent='out')
    asymp_relative_infect = xs.foreign(KEYS_MAPPING['asymp_relative_infect'], 'asymp_relative_infect', intent='out')
    asymp_rate = xs.foreign(KEYS_MAPPING['asymp_rate'],
                            'asymp_rate', intent='out')
    hosp_f_ratio = xs.foreign(KEYS_MAPPING['hosp_f_ratio'], 'hosp_f_ratio', intent='out')
    prop_trans_in_p = xs.foreign(KEYS_MAPPING['prop_trans_in_p'], 'prop_trans_in_p', intent='out')
    symp_h_ratio = xs.foreign(KEYS_MAPPING['symp_h_ratio'],
                              'symp_h_ratio', intent='out')
    symp_h_ratio_w_risk = xs.foreign(KEYS_MAPPING['symp_h_ratio_w_risk'], 'symp_h_ratio_w_risk', intent='out')
